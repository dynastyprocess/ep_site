...
ep <- arrow::read_parquet("data/expected_points_2021.pdata") %>%
  ...
  group_by(player_id) %>% 
  mutate(sortby = -mean(total_fantasy_points_exp, na.rm = TRUE)) %>%
  ungroup() %>% 
  select(-player_id) %>% 
...
table_ep <- crosstalk_ep %>%
  reactable(
    ...
    groupBy = "Display",
    columns = list(
      `Display` = colDef(name = "Player", minWidth = 150,sticky = "left"),
      Sortby = colDef(show = FALSE),
      `Pos` = colDef(show = FALSE),
      `Team` = colDef(show = FALSE),
      `Player Name` = colDef(show = FALSE),
      `Week` = colDef(minWidth = 30,aggregate = NULL, format = colFormat(digits = 0)),
...
bscols(
  ...
  filter_select("week", "Week", crosstalk_ep, ~`Week`),
  filter_slider("ep", "EP", crosstalk_ep, ~-`Sortby`)
