...
ep <- arrow::read_parquet("data/expected_points_2021.pdata") %>%
  select(
    ...
    pos = position,
    team = posteam,
    contains("total_fantasy_points"),
    contains("rec_fantasy_points"),
    contains("rush_fantasy_points"),
    contains("pass_fantasy_points"),
...
```{r layout = "l-screen", fig.width = 16, fig.height = 16}
...
table_ep <- crosstalk_ep %>%
  reactable(
    ...
    compact = TRUE,
    columns = list(
      # `Season` = colDef(minWidth = 75,
      #                   format = colFormat(digits = 0)),
      `Games` = colDef(name = "GP", format = colFormat(digits = 0)),
      `Player Name` = colDef(minWidth = 100)
    ),
